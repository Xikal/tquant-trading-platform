...
if test -n "${PLATFORM_AUTOPILOT_ENABLED:-}"; then
  upsert_env_value PLATFORM_AUTOPILOT_ENABLED "$PLATFORM_AUTOPILOT_ENABLED"
fi
if test -n "${RUNTIME_LOW_PRIORITY_TASKS_PAUSED:-}"; then
  upsert_env_value RUNTIME_LOW_PRIORITY_TASKS_PAUSED "$RUNTIME_LOW_PRIORITY_TASKS_PAUSED"
fi
if test -n "${MARKET_REVIEW_ENABLED:-}"; then
  upsert_env_value MARKET_REVIEW_ENABLED "$MARKET_REVIEW_ENABLED"
fi
if test -n "${DATA_QUALITY_SLA_ENABLED:-}"; then
  upsert_env_value DATA_QUALITY_SLA_ENABLED "$DATA_QUALITY_SLA_ENABLED"
fi
if test -n "${RUNTIME_BACKGROUND_COMPACT_MODE_ENABLED:-}"; then
  upsert_env_value RUNTIME_BACKGROUND_COMPACT_MODE_ENABLED "$RUNTIME_BACKGROUND_COMPACT_MODE_ENABLED"
fi
if test -n "${RUNTIME_STARTUP_CACHE_PREWARM_ENABLED:-}"; then
  upsert_env_value RUNTIME_STARTUP_CACHE_PREWARM_ENABLED "$RUNTIME_STARTUP_CACHE_PREWARM_ENABLED"
fi
if test -n "${RUNTIME_STARTUP_HISTORY_PREWARM_ENABLED:-}"; then
  upsert_env_value RUNTIME_STARTUP_HISTORY_PREWARM_ENABLED "$RUNTIME_STARTUP_HISTORY_PREWARM_ENABLED"
fi
if test -n "${RUNTIME_WORKER_RECYCLE_RSS_MB:-}"; then
  upsert_env_value RUNTIME_WORKER_RECYCLE_RSS_MB "$RUNTIME_WORKER_RECYCLE_RSS_MB"
fi
if embedded_scheduler_enabled; then
