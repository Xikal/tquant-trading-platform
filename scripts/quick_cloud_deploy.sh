...
PLATFORM_AUTOPILOT_ENABLED="${PLATFORM_AUTOPILOT_ENABLED:-}"
RUNTIME_LOW_PRIORITY_TASKS_PAUSED="${RUNTIME_LOW_PRIORITY_TASKS_PAUSED:-}"
MARKET_REVIEW_ENABLED="${MARKET_REVIEW_ENABLED:-}"
DATA_QUALITY_SLA_ENABLED="${DATA_QUALITY_SLA_ENABLED:-}"
RUNTIME_BACKGROUND_COMPACT_MODE_ENABLED="${RUNTIME_BACKGROUND_COMPACT_MODE_ENABLED:-}"
RUNTIME_STARTUP_CACHE_PREWARM_ENABLED="${RUNTIME_STARTUP_CACHE_PREWARM_ENABLED:-}"
RUNTIME_STARTUP_HISTORY_PREWARM_ENABLED="${RUNTIME_STARTUP_HISTORY_PREWARM_ENABLED:-}"
RUNTIME_WORKER_RECYCLE_RSS_MB="${RUNTIME_WORKER_RECYCLE_RSS_MB:-}"
...
if [[ -n "$PLATFORM_AUTOPILOT_ENABLED" ]]; then
  export PLATFORM_AUTOPILOT_ENABLED
fi
if [[ -n "$RUNTIME_LOW_PRIORITY_TASKS_PAUSED" ]]; then
  export RUNTIME_LOW_PRIORITY_TASKS_PAUSED
fi
if [[ -n "$MARKET_REVIEW_ENABLED" ]]; then
  export MARKET_REVIEW_ENABLED
fi
if [[ -n "$DATA_QUALITY_SLA_ENABLED" ]]; then
  export DATA_QUALITY_SLA_ENABLED
fi
if [[ -n "$RUNTIME_BACKGROUND_COMPACT_MODE_ENABLED" ]]; then
  export RUNTIME_BACKGROUND_COMPACT_MODE_ENABLED
fi
if [[ -n "$RUNTIME_STARTUP_CACHE_PREWARM_ENABLED" ]]; then
  export RUNTIME_STARTUP_CACHE_PREWARM_ENABLED
fi
if [[ -n "$RUNTIME_STARTUP_HISTORY_PREWARM_ENABLED" ]]; then
  export RUNTIME_STARTUP_HISTORY_PREWARM_ENABLED
fi
if [[ -n "$RUNTIME_WORKER_RECYCLE_RSS_MB" ]]; then
  export RUNTIME_WORKER_RECYCLE_RSS_MB
fi
...
if [[ -n "$PLATFORM_AUTOPILOT_ENABLED" ]]; then
  deploy_env_args+=(PLATFORM_AUTOPILOT_ENABLED="$PLATFORM_AUTOPILOT_ENABLED")
fi
if [[ -n "$RUNTIME_LOW_PRIORITY_TASKS_PAUSED" ]]; then
  deploy_env_args+=(RUNTIME_LOW_PRIORITY_TASKS_PAUSED="$RUNTIME_LOW_PRIORITY_TASKS_PAUSED")
fi
if [[ -n "$MARKET_REVIEW_ENABLED" ]]; then
  deploy_env_args+=(MARKET_REVIEW_ENABLED="$MARKET_REVIEW_ENABLED")
fi
if [[ -n "$DATA_QUALITY_SLA_ENABLED" ]]; then
  deploy_env_args+=(DATA_QUALITY_SLA_ENABLED="$DATA_QUALITY_SLA_ENABLED")
fi
if [[ -n "$RUNTIME_BACKGROUND_COMPACT_MODE_ENABLED" ]]; then
  deploy_env_args+=(RUNTIME_BACKGROUND_COMPACT_MODE_ENABLED="$RUNTIME_BACKGROUND_COMPACT_MODE_ENABLED")
fi
if [[ -n "$RUNTIME_STARTUP_CACHE_PREWARM_ENABLED" ]]; then
  deploy_env_args+=(RUNTIME_STARTUP_CACHE_PREWARM_ENABLED="$RUNTIME_STARTUP_CACHE_PREWARM_ENABLED")
fi
if [[ -n "$RUNTIME_STARTUP_HISTORY_PREWARM_ENABLED" ]]; then
  deploy_env_args+=(RUNTIME_STARTUP_HISTORY_PREWARM_ENABLED="$RUNTIME_STARTUP_HISTORY_PREWARM_ENABLED")
fi
if [[ -n "$RUNTIME_WORKER_RECYCLE_RSS_MB" ]]; then
  deploy_env_args+=(RUNTIME_WORKER_RECYCLE_RSS_MB="$RUNTIME_WORKER_RECYCLE_RSS_MB")
fi
env "${deploy_env_args[@]}" "$ROOT_DIR/scripts/deploy_cloud_server.sh"
