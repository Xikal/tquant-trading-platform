    ...
    steps = [
        ...
        step(
            ...
            [
                ...
                remote("sudo docker compose -f docker-compose.mysql.yml up -d app runtime-scheduler runtime-worker"),
            ],
            destructive=False,
        ...
        step(
            ...
            "预演 runtime-worker 内嵌 scheduler 切换命令",
            [
                remote("grep -Eq '^RUNTIME_WORKER_EMBED_SCHEDULER=' .env && grep -Eq '^RUNTIME_SCHEDULER_BACKGROUND_JOBS_ENABLED=' .env || true"),
                remote("sudo docker compose -f docker-compose.mysql.yml config >/tmp/tquant-embedded-scheduler-compose.yml"),
                f"python3 scripts/verify_platform_budget.py {ssh_args} --project-dir {remote_project_dir} --json-output docs/reports/platform-budget-embedded-scheduler-dry-run-${{DATE}}.json --markdown-output docs/reports/platform-budget-embedded-scheduler-dry-run-${{DATE}}.md",
            ],
            destructive=False,
        ),
        step(
            "embedded_runtime_scheduler_apply",
            "低峰窗口启用 runtime-worker 内嵌 scheduler 并停独立 scheduler",
            [
                remote("cp .env .env.scheduler-backup-${DATE}"),
                remote("python3 - <<'PY'\nfrom pathlib import Path\npath = Path('.env')\npairs = {'RUNTIME_WORKER_EMBED_SCHEDULER': 'true', 'RUNTIME_SCHEDULER_BACKGROUND_JOBS_ENABLED': 'false'}\nlines = path.read_text(encoding='utf-8').splitlines()\nseen = set()\nout = []\nfor line in lines:\n    key = line.split('=', 1)[0] if '=' in line else ''\n    if key in pairs:\n        out.append(f'{key}={pairs[key]}')\n        seen.add(key)\n    else:\n        out.append(line)\nfor key, value in pairs.items():\n    if key not in seen:\n        out.append(f'{key}={value}')\npath.write_text('\\n'.join(out) + '\\n', encoding='utf-8')\nPY"),
                remote("sudo docker compose -f docker-compose.mysql.yml up -d --no-deps --force-recreate runtime-worker"),
                remote("sudo docker rm -f tquant-runtime-scheduler-mysql"),
            ],
            destructive=False,
            requires_operator=True,
            rollback=remote("cp .env.scheduler-backup-${DATE} .env && sudo docker compose -f docker-compose.mysql.yml up -d --no-deps --force-recreate runtime-worker runtime-scheduler"),
        ),
        step(
