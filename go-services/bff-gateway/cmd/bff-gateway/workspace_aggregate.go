...
func aggregateMonitorWorkspace(cfg config, client *http.Client, r *http.Request) aggregateResult {
	...
	if view == "action" {
		...
		breadthTimeout := 250 * time.Millisecond
		reviewTimeout := 120 * time.Millisecond
		sectorTimeout := 250 * time.Millisecond
		hedgeTimeout := 120 * time.Millisecond
		if view == "market" {
			breadthTimeout = 4 * time.Second
			reviewTimeout = 2 * time.Second
			sectorTimeout = 4 * time.Second
			hedgeTimeout = 2 * time.Second
		}
		sources = append(
			sources,
			rawSource{name: "market_breadth", path: "/api/market/breadth", query: values("realtime", "true"), timeout: breadthTimeout},
			rawSource{name: "monitor_review", path: "/api/market/review-summary", timeout: reviewTimeout},
			...
			), timeout: sectorTimeout},
			rawSource{name: "paired_hedge", path: "/api/market/paired-hedge-research", query: values("limit", queryDefault(q, "hedge_limit", "4")), timeout: hedgeTimeout},
		)
	}
