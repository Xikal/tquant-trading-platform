# first_board OOS 诊断与回吐池报告（2026-06-11）
...
## 核心内容
- first_board OOS：成交 `46`，PF `0.7516`，收益 `-1.0111%`。
- 诊断状态：`blocked_by_data`，结论：数据不足，阻断调参。
- 回吐池状态：`blocked_by_data`；未使用 `max_gain_5d` 作为生产晋级收益。

批次结论：`blocked_by_data`；生产变更允许：`False`。

## 机器产物
- JSON：`backend/data/reports/strategy-success-rate/first-board-oos-diagnosis-2026-06-11.json`
- Markdown：`docs/reports/first-board-oos-diagnosis-2026-06-11.md`
