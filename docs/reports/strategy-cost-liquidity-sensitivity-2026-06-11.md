...
## 核心内容
- 成本分桶状态：`blocked_by_data`。
- `portfolio_backtest_metrics(extra_cost_bps)` 默认行为未变。

批次结论：`blocked_by_data`；生产变更允许：`False`。

## 机器产物
- JSON：`backend/data/reports/strategy-success-rate/strategy-cost-liquidity-sensitivity-2026-06-11.json`
- Markdown：`docs/reports/strategy-cost-liquidity-sensitivity-2026-06-11.md`
