# volume_shrink 分时确认研究（2026-06-11）
...
## 核心内容
- volume_shrink 分时确认状态：`partial_minute_coverage`，分钟覆盖 `0.0`%。
- 未把 `volume_shrink` 加入生产 VWAP/尾盘确认集合。

批次结论：`partial_minute_coverage`；生产变更允许：`False`。

## 机器产物
- JSON：`backend/data/reports/strategy-success-rate/strategy-volume-shrink-intraday-confirmation-2026-06-11.json`
- Markdown：`docs/reports/strategy-volume-shrink-intraday-confirmation-2026-06-11.md`
