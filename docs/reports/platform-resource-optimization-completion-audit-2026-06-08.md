...
- 生成时间：`2026-06-08T00:56:05.044725+00:00`
- 总状态：`blocking`
...
| phase_a | 根分区 >80% blocking，长期目标 <60%。 | `passed` | root_used_pct=56 | 继续每日巡检；长期目标仍需维持在 60% 附近。 |
| phase_a | Available memory >800MiB，或有明确单机不可达原因。 | `warning` | available_mb=219 | 资源上限和 worker 降载在线生效后复验；若仍低于 800MiB，需在验收报告写明单机不可达原因。 |
| phase_a | MySQL binlog 固化保留 3 天。 | `passed` | binlog_expire_logs_seconds=259200 |  |
| phase_a | max_binlog_size 目标 256M。 | `blocked` | max_binlog_size=1073741824 | 运维窗口重建 MySQL 容器使 compose max-binlog-size 生效，并复验 @@max_binlog_size。 |
| phase_a | MySQL slow log 单文件 <=256M 并安装 logrotate。 | `blocked` | slow_log_bytes=3328599654; mysql_slow_logrotate_missing=True | 先备份 slow log，再安装 logrotate 并执行轮转/复验。 |
| phase_a | systemd journal 上限 300M/7day 在线生效。 | `blocked` | journal_usage_bytes=1288490188; journald_dropin_missing=True | 运维窗口安装 journald drop-in 并重启/复验 journal 占用。 |
| phase_a | Docker BuildKit cache 目标 <=2G。 | `blocked` | docker_build_cache_bytes=6400575012 | 启用 BuildKit GC 或只清 build cache；禁止清 volume。 |
| phase_a | Docker/journal/BuildKit/MySQL compose command/slow logrotate 配置在线安装或生效并可复验。 | `blocked` | resource:resource_config_missing=buildkit, resource:resource_config_missing=docker_daemon, resource:resource_config_missing=journald_dropin, resource:resource_config_missing=mysql_compose_resource_config, resource:resource_config_missing=mysql_slow_logrotate | 按维护窗口计划执行 resource_limits_apply 后重跑 baseline/readiness。 |
| phase_a | MySQL volume 短期 <12G，长期通过 Parquet/manifest 控制增长。 | `passed` | mysql_volume_bytes=11811160064.0 | manifest 全部补齐并完成保留窗口 dry-run 前，不清 MySQL 源数据。 |
...
| phase_b | /next/paper 长列表接 VirtualList，DOM <150，机甲组件不改。 | `warning` | dom_nodes=343, non_mecha_descendants=82, mecha_descendants=171 | 若坚持整页 DOM <150，需要另行授权简化机甲组件 DOM。 |
...
| phase_c | MySQL max_connections 收敛到 80-120 并在线生效。 | `blocked` | max_connections=300, threads_connected=26 | 运维窗口重启 MySQL/容器，使 compose 目标 120 生效并复跑 performance gate。 |
| phase_c | 低优先级重任务暂停开关进入运行容器。 | `blocked` | worker_budget:low_priority_pause_env_missing=analytics_worker, worker_budget:low_priority_pause_env_missing=backtest_worker, worker_budget:low_priority_pause_env_missing=runtime_scheduler, worker_budget:low_priority_pause_env_missing=runtime_worker | 运维窗口重启 runtime/backtest/analytics/scheduler 容器后复验。 |
...
| phase_c | Swap 长期 <20%，短期优化目标 <500MiB。 | `blocked` | swap_used_pct=100.0 | 资源上限和 worker 降载在线生效后复验。 |
| phase_d | daily_bars Parquet manifest 可校验。 | `passed` | ready_count=1, verified_files=25 |  |
| phase_d | strategy/backtest/report 等目标数据集 manifest 全部补齐。 | `blocked` | missing_count=9, blocked_count=9, blocked_datasets=['strategy_tracking_snapshots', 'key_level_snapshots', 'low_buy_result_snapshots', 'backtest_runs', 'backtest_trades', 'backtest_daily_snapshots', 'analysis_logs', 'market_review_reports', 'paper_review_reports'] | 按导出计划显式入队 9 个 analytics export 任务，完成后重跑 manifest verifier。 |
...
| phase_f | 线上 Web/API performance gate 当前样本通过。 | `passed` | ok=True, failures=[], readyz p95=7.336ms status=[200], monitor_bff p95=30.266ms status=[200], market_pulse p95=28.294ms status=[200], priority_board p95=103.424ms status=[200], watchlist_signals p95=24.109ms status=[200] | 资源 apply、worker 重启、manifest 导出后必须重新跑三轮 online performance gate。 |
| phase_f | 报告闭环包含 before/after、资源、MySQL、Worker、manifest、回滚、未完成项。 | `passed` | acceptance_status=blocking, open_items=19 |  |
