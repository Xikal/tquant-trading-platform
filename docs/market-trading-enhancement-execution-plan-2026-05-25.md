...
- Go/Rust 新能力在验收前可用 shadow 或受控开关隔离；验收完成后必须接入生产主路径，并保留一键回退 Python 的能力。
...
| M3 | Go 服务生产化 | P1 | bff-gateway 聚合、market-read 批量读、scan-worker 扫描任务、性能门禁、生产主路径接入 | M0 | 1-2 周 |
| M4 | 数据迁移与 settings 原子化 | P1/P2 | 迁移 scope 明确，settings 持久化避免半状态 | M0 | 1 周 |
| M5 | Rust 内核生产化 | P2 | RSI/VWAP/RankIC、wheel、CI 验收、fallback、生产计算主路径接入 | M0/M3 | 1-2 周 |
| M6 | 历史表、CI、观测与样式收敛 | P2/P3 | 历史留痕、Go/Rust 门禁、可观测降级、前端收敛 | M1-M5 | 持续迭代 |
...
| M1-FE-2 | 监控页 pulse 条 | `ui-designer` / `fullstack-builder` | `frontend/src/features/monitor/` | 首屏 pulse strip/card | 375/768/1440 无遮挡 |
...
| M1-T-4 | 响应式截图测试 | 375/768/1440 宽度 |
...
| M2-FE-2 | 实时监控复盘卡片 | `ui-designer` / `fullstack-builder` | `frontend/src/features/monitor/` | 午盘、收盘、风险提示、建议动作 | 内容紧凑，无大面积留白 |
...
把 Go 三个服务做成生产主路径能力。验收完成后，`bff-gateway` 承担 workspace/monitor 聚合主路径，`market-read-service` 承担监控、情绪、选股的行情读取主路径，`scan-worker` 承担全市场扫描、候选排序和 snapshot 写入主路径；Python 保留 fallback 和业务真源。
...
| M3-GO-7 | `scan-worker` 生产扫描任务 | `fullstack-builder` / `stock-analysis-specialist` | `go-services/scan-worker` | 全市场扫描、候选排序、snapshot 写入 | 候选排序与 Python reference 一致 |
...
| M3-PY-4 | scan-worker 结果接入 | `fullstack-builder` | scan result repository / monitor API | 读取 Go 写入 snapshot | Python reference 可对比 |
| M3-PY-5 | 配置和 compose 对齐 | `devops-operator` | `docker-compose.mysql.yml`、`.env.docker.example` | Go DSN/token/URL 配置完整 | profile 启用即能工作 |
| M3-PY-6 | 生产主路径切换 | `fullstack-builder` | market data service、BFF、settings、scheduler | 验收完成后默认优先调用 Go BFF/read/scan 路径 | 关闭 Go 配置可回退 Python |
...
| M3-T-5 | scan-worker 排序一致性测试 | Go 候选排序与 Python reference 对齐 |
| M3-T-6 | scan-worker snapshot 写入测试 | 写入幂等、失败不污染 latest |
...
- `scan-worker` 支持全市场扫描、候选排序和 snapshot 写入主路径。
- 质量字段可被前端正确消费。
- Go 关闭后 Python fallback 完整可用。
- 性能门禁进入 CI 或发布前脚本。
- 生产配置启用 Go 后，workspace/监控/情绪/选股/扫描请求默认命中 Go 主路径。
...
补齐核心指标、修复 Python wrapper、发布 wheel，把 Rust 验收纳入 CI，并在验收完成后接入生产计算主路径。
...
| M5-PY-2 | 生产 feature flag | `devops-operator` | config/settings | 验收前可关闭，验收后生产默认启用 | 关闭即走 Python |
| M5-PY-3 | 回测/因子实验室生产接入 | `fullstack-builder` | finance/backtest/factor services | 指标热点默认走 Rust | 策略语义不变 |
...
| M5-T-5 | 生产路径命中测试 | Rust 启用时默认命中 Rust，关闭时回退 Python |
...
| M6-CI-2 | 前端截图 smoke | `qa-tester` | scripts/tests | 375/768/1440 截图 | 页面无遮挡 |
...
| ID | 任务 | 责任角色 | 改动范围 | 产出 | 验收 |
|---|---|---|---|---|---|
| M6-FE-1 | 页面级 style 收敛 | `ui-designer` / `fullstack-builder` | monitor/paper/backtest/settings | 共享 tokens/components | 样式更统一 |
| M6-FE-2 | 大 chunk 拆分 | `fullstack-builder` | Vite chunk config/lazy imports | 降低首屏体积 | build warning 可控 |
| M6-FE-3 | 移动端入口补齐 | `ui-designer` | mobile shell | 监控/情绪/模拟盘/设置入口一致 | 移动体验闭环 |

### Definition of Done

- 历史快照和复盘可查询。
- CI 覆盖 Go/Rust 和前端基础截图。
...
- 功能开关默认用于开发隔离。
- Go/Rust 先在开发环境验证生产主路径命中和 fallback。
- 前端页面先展示但不驱动交易动作。

### 阶段二：灰度验证

- 开启 pulse 和复盘展示。
- Go BFF/read/scan 接入一部分 workspace、监控、情绪、选股和扫描生产请求，并记录命中率。
- Rust 接入一部分回测/因子/性能评估计算热点，并记录命中率。
...
- 运行手册和回滚路径完整。
- 连续交易日无策略结果异常。
- Go BFF/read/scan 成为 workspace、监控、情绪、选股和扫描主路径。
- Rust 成为已迁移指标的生产计算主路径。
...
| Go BFF/read/scan | 清空服务 URL 或关闭 Go feature flag，回退 Python |
| Rust 内核 | 关闭 `RUST_FINANCE_MATH_ENABLED` |
...
- [ ] Go `bff-gateway`、`market-read-service`、`scan-worker` 已接入生产主路径，并有命中/回退指标。
- [ ] Rust 已接入生产计算主路径，并有命中/回退指标。
- [ ] settings 写失败不会留下半状态。
