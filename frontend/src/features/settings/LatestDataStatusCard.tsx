...
function latestDataView(status: LatestLowBuyDataStatus | null): {
  ...
  const freshness = buildDataFreshnessView({
    ...
    stale: (status.local_staleness_trade_days ?? 0) > 0 || (status.staleness_trade_days ?? 0) > 0,
    staleReason: latestDataStaleReason(status),
    latestTradeDate: status.published_trade_date || status.local_latest_trade_date,
    expectedTradeDate: status.calendar_expected_trade_date || status.expected_trade_date,
    missingCount: status.missing_strategies?.length ?? 0,
  });
  return {
    title: freshness.status === "ok" ? "最新数据已更新" : freshness.title,
    description: freshness.status === "ok" ? "前端榜单和选股宝典可使用已发布交易日数据。" : freshness.detail,
    tone: freshness.tone,
  };
}

function latestDataStaleReason(status: LatestLowBuyDataStatus): string {
  if ((status.local_staleness_trade_days ?? 0) > 0) {
    return `本地日线停留在 ${status.local_latest_trade_date || "--"}，距目标交易日 ${status.calendar_expected_trade_date || status.expected_trade_date || "--"} 落后 ${status.local_staleness_trade_days} 个交易日。`;
  }
  if ((status.staleness_trade_days ?? 0) > 0) {
    return `已发布交易日 ${status.published_trade_date || "--"} 距目标交易日 ${status.calendar_expected_trade_date || status.expected_trade_date || "--"} 落后 ${status.staleness_trade_days} 个交易日。`;
  }
  return "";
}
