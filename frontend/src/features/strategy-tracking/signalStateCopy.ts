...
export function signalStateKindText(signalState: string): string {
  ...
  if (signalState === "near_entry") return "观察类：接近买点，不是买入";
  if (signalState === "observe_confirmed") return "观察类：提醒观察，不是买入";
  return "跟踪类：仅用于复盘";
}
...
export function signalStateHelpText(signalState: string): string {
  ...
  if (signalState === "near_entry") return "接近买点只代表快到观察区，不是买入建议，不能提前买。";
  if (signalState === "observe_confirmed") return "观察确认只用于后续触发，不是买入建议。";
...
function signalStateLabel(signalState: string): string {
  ...
  if (signalState === "observe_confirmed") return "观察确认（提醒）";
