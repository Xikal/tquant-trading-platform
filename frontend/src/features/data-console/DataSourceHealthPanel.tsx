  ...
  return (
    <div className={styles.panelBody}>
      <div className={styles.toolbar}>
        <span className={styles.muted}>最近探测：{data?.updated_at ?? "--"}</span>
        <Button size="small" onClick={onRefresh} loading={loading}>重新探测</Button>
      </div>
      {error ? <Alert type="error" showIcon message={error} /> : null}
      {!items.length && !loading && !error ? <div className={styles.empty}>数据源探测暂无返回</div> : null}
          ...
          return (
            <div className={styles.sourceRow} key={item.source}>
              <div>
                ...
                <div className={styles.detail}>{unhealthy ? item.warning || SOURCE_UNAVAILABLE_TEXT : item.warning || "数据源在线"}</div>
              </div>
              <div className={styles.numbers}>
                <Tag color={unhealthy ? "red" : item.quality === "degraded" || item.quality === "stale" ? "orange" : "green"}>{item.quality}</Tag>
