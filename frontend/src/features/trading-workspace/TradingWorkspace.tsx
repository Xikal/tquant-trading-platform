...
export function TradingWorkspace() {
  ...
  async function runPriorityAi() {
    ...
    await withLoading("ai", async () => {
      const priorityItems = monitor.priorityBoard?.items ?? [];
      const result = await api.buildAiDecisionSupport({
        ...
        payload: {
          ...
          total_candidates: monitor.priorityBoard?.total_candidates ?? priorityItems.length,
          immediate_count: monitor.priorityBoard?.immediate_count ?? 0,
