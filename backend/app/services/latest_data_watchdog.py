...
class LatestDailyBarWatchdog:
    ...
    def run(
        ...
        *,
        ...
        if event is not None and int(event.notification_count or 0) > 0 and not force_notify:
        ...
        event.signal_state = "ok" if complete else "missing_daily_bars"
...
class LatestDataWatchdogLedger:
    ...
    def already_notified(self, *, trade_date: str, channel: str = "feishu") -> bool:
        ...
        return bool(event is not None and int(event.notification_count or 0) > 0)
...
def _message(*, expected_trade_date: str, daily_count: int, complete: bool, checked_at: datetime) -> str:
    status = "日线已更新" if complete else "日线未更新"
    level = "正常" if complete else "告警"
    action = "低吸榜可继续等待策略物化结果。" if complete else "请检查 runtime-worker、数据源和 daily_bar_refresh 任务。"
    return "\n".join(
        [
