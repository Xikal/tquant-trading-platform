...
def build_exit_context(quote: PaperQuotePrice | None, bars: list[Any] | None) -> PaperExitContext:
    ...
    volume_release_ratio = _volume_release_ratio(cleaned_bars)
    ...
    return PaperExitContext(
        ...
        reason=_context_reason(above_vwap, reclaimed_vwap, vwap_hold, volume_release_ratio, high_pullback_ratio),
    )
...
def _volume_release_ratio(bars: list[Any]) -> float:
...
def _context_reason(above_vwap: bool, reclaimed_vwap: bool, vwap_hold: bool, volume_ratio: float, pullback_ratio: float) -> str:
    if not above_vwap:
