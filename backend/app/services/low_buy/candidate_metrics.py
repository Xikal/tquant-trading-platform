...
def build_candidate_metrics(
    ...
    divergence_metrics = _build_divergence_consensus_metrics(
        history=history,
        board_index=board_index,
        latest_index=latest_index,
        board_row=board_row,
...
def _build_divergence_consensus_metrics(
    history: pd.DataFrame,
    board_index: int,
    latest_index: int,
    board_row: pd.Series,
    ...
    if pre_latest.empty:
        return _empty_divergence_consensus_metrics(board_high, board_volume, latest)
    ...
    divergence_label, divergence_day_stall = _select_divergence_day(
    ...
    close_strength = _close_strength(latest)
    latest_close = float(latest["close"])
    latest_open = float(latest["open"])
    latest_change_pct = float(latest["pct_chg"])
    return {
...
def _empty_divergence_consensus_metrics(board_high: float, board_volume: float, latest: pd.Series) -> dict[str, float | int | bool]:
    return {
        ...
        "consensus_close_strength": _close_strength(latest),
    }


def _select_divergence_day(
    ...
    for label, row in divergence_window.iterrows():
        if _is_divergence_stall_day(row=row, board_row=board_row, board_volume=board_volume):
    ...
    return fallback_label, _is_divergence_stall_day(
        row=fallback_row,
        board_row=board_row,
        board_volume=board_volume,
    )
...
def _is_divergence_stall_day(row: pd.Series, board_row: pd.Series, board_volume: float) -> bool:
    ...
    close_strength = _close_strength(row)
