...
PLAYBOOKS: dict[str, dict[str, Any]] = {
    ...
    "late_session_strong_support": {
        "title": "收盘强势承接",
        "subtitle": "辅助生产 · 低样本限权，日线强收盘 + 盘中均价确认",
    ...
    "core_midcap_vwap_ma5_retrace": {
        "title": "中军VWAP/均线回踩",
        "subtitle": "研究验证 · 主线中军回踩 5/10 日线，盘中站回 VWAP 才执行",
    ...
    "sector_mainline_first_divergence_low_buy": {
        "title": "主线首分歧低吸",
        "subtitle": "研究验证 · 板块主线第一次分歧，只做核心前排",
    ...
    "mainline_limitup_shrink_retrace_reclaim": {
        "title": "主线涨停缩量回调",
        "subtitle": "研究验证 · 主线涨停后 3-8 日缩量回踩，不追高",
    ...
    "ma_channel_band": {
        "title": "均线通道波段",
        "subtitle": "研究验证 · MA20 通道下轨承接，上轨兑现",
        "logic": "只研究沿 MA20 通道运行的趋势票，重点观察回踩通道下沿或 MA20 附近后的缩量承接。",
        "notes": [
            "当前定位为研究/回测策略，不进入生产强买入口。",
            "必须保持中期趋势，回踩 MA20 附近不能放量破位。",
            "以 5-15 天波段验证为主，达标前只输出研究样本。",
        ],
    },
    "leader_pullback_band": {
        "title": "龙头回踩波段",
        "subtitle": "研究验证 · 主线龙头确认后回踩均线支撑",
        "logic": "只研究热点主线龙头或强跟随在启动确认后的二波回踩结构，要求趋势未破且承接仍在。",
        "notes": [
            "当前定位为高风险研究策略，探针未完成前不开放生产入口。",
            "必须是主线或次主线核心票，后排补涨不纳入。",
            "跌破关键均线或板块退潮时直接判定失败。",
        ],
    },
    "n_pattern_long_wash": {
        "title": "长洗N字冲高",
        "subtitle": "研究归档 · 24M 最大回撤过高，不进入生产优先榜",
        "logic": "先识别涨停/大阳启动，再验证 7-15 日缩量回落是否始终守住启动日低点；重新放量中阳或连续小阳爬升后才参与。",
        "notes": [
            "当前定位为研究观察策略，不进入生产强买或模拟盘生产入口。",
            "核心防守位是启动日低点，跌破即判定 N 字结构失败。",
            "洗盘越久越要看量能是否干净收缩，盈利主要来自 3-5 日冲高兑现。",
        ],
    },
    "n_pattern_short_wash": {
        "title": "短洗N字冲高",
        "subtitle": "研究归档 · 24M 结果为删除候选，不进入生产优先榜",
        "logic": "涨停或大阳启动后，2-5 日内出现放量分歧并快速缩回；尾盘收红十字、锤头或长下影且不破启动低点，才允许小仓参与。",
        "notes": [
            "当前定位为高风险研究归档策略，不进入生产强买或模拟盘生产入口。",
            "只适合尾盘小仓参与，次日不弱转强就按 T+1 纪律退出。",
            "若出现长上影、放量阴跌或跌破启动低点，直接放弃。",
        ],
    },
    "breakout_support": {
        "title": "位置支撑",
